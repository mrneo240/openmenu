 ...
 * File: ui_bios.c
 * Project: ui
 * File Created: Thursday, 13th October 2022 6:30:57 pm
...
static const int pos_gameinfo_x = 404;
...
/* GDMENU Strings */
/* English */
#define INFO_STR_LEN (16) /* Amount of characters needed to fill space */
...
static void draw_bg_layers(void) {
  draw_draw_quad(0, 0, 512, 480, COLOR_WHITE);
  draw_draw_quad(512, 0, 128, 480, COLOR_WHITE);
...
static void draw_gamelist(void) {
  ...
  const int Y_ADJUST_CRSR = -1; /* 2 pixels higher than text */
  font_bios_begin_draw();
  if (list_len <= 0) {
    ...
    font_bios_set_color(color_main_highlight);
    font_bios_draw_main(pos_gameslist_x + X_ADJUST_TEXT, pos_gameslist_y + Y_ADJUST_TEXT + (0 * 21), "Empty Game List!");
  }
  ...
  for (int i = 0; i < items_per_page; i++) {
    ...
    if ((current_starting_index + i) == current_selected_item) {
      ...
      font_bios_set_color(highlight_text_color);
    } else {
      font_bios_set_color(color_main_default);
    }

    font_bios_draw_main(pos_gameslist_x + X_ADJUST_TEXT, pos_gameslist_y + Y_ADJUST_TEXT + (i * 21), buffer);
  }
}
...
static void draw_gameinfo(void) {
  ...
  font_bios_draw_main(pos_gameinfo_x, pos_gameinfo_date_y, line_buf);
  // VERSION
  string_outer_concat(line_buf, STR_VERSION, list_current[current_selected_item]->version, INFO_STR_LEN);
  font_bios_draw_main(pos_gameinfo_x, pos_gameinfo_version_y, line_buf);
}
...
FUNCTION(UI_NAME, init) {
  ...
  texman_reserve_memory(txr_bg_left.width, txr_bg_left.height, 2);

  temp = texman_create();
  draw_load_texture_buffer("THEME/GDMENU/BG_R.PVR", &txr_bg_right, texman_get_tex_data(temp));
  texman_reserve_memory(txr_bg_right.width, txr_bg_right.height, 2);
